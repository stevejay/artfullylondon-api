...
import * as searchBuilders from "./search-builders";
...
describe("buildTalentRelatedEventsSearchPreset", () => {
  it("should build a correct search", () => {
    ...
    expect(result).toEqual([
      { index: searchIndexType.EVENT_FULL, type: "doc" },
      {
        _source: [
          "entityType",
          "id",
...
describe("buildEntityCountsSearchPreset", () => {
  it("should build an entity counts search", () => {
    ...
    expect(result).toEqual([
      ...
      { size: 0 },
      { index: searchIndexType.TALENT_FULL, type: "doc" },
      { size: 0 },
      { index: searchIndexType.VENUE_FULL, type: "doc" },
      { size: 0 }
    ]);
  });
});
        ...
        query: {
          bool: {
            filter: [
              {
                terms: {
                  externalEventId: ["almeida-theatre|/a", "almeida-theatre|/b"]
                }
              }
            ]
          }
  ...
  it("should build a featured events search", () => {
    ...
    expect(result).toEqual([
      { index: searchIndexType.EVENT_FULL, type: "doc" },
      {
        ...
        sort: [{ _score: { order: "desc" } }, { name_sort: { order: "asc" } }],
        query: {
          bool: {
            filter: [
              { term: { status: "Active" } },
