...
const normalisers = require('../../lib/event/normalisers');

describe('event normalisers', () => {
  it('should apply event normalisers to fully populated params', () => {
    const params = {
      ...
      description: '  <p>A contemporary update of this Shakespeare classic by the acclaimed director Sam Mendes.</p>  ',
  ...
  it('should apply event normalisers to times audience tags', () => {
    ...
    expect(params.specialOpeningTimes[0].audienceTags).to.eql(undefined);
    expect(params.specialPerformances[0].audienceTags).to.eql(undefined);
    expect(params.images[0].copyright).to.eql(undefined);
    expect(params.images[0].dominantColor).to.eql(undefined);
  });
});
