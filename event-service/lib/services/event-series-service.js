...
module.exports.getEventSeriesMulti = co.wrap(function*(eventSeriesIds) {
  const response = yield dynamoDbClient.batchGet({
    RequestItems: {
      [process.env.SERVERLESS_EVENT_SERIES_TABLE_NAME]: {
        Keys: eventSeriesIds.map(id => ({ id })),
        ProjectionExpression: constants.SUMMARY_EVENT_SERIES_PROJECTION_EXPRESSION,
        ExpressionAttributeNames: constants.SUMMARY_EVENT_SERIES_PROJECTION_NAMES,
      },
    },
    ReturnConsumedCapacity: process.env.RETURN_CONSUMED_CAPACITY,
  });

  const dbItems = response.Responses[
    process.env.SERVERLESS_EVENT_SERIES_TABLE_NAME
  ];
...
module.exports.getEventSeriesForEdit = co.wrap(function*(eventSeriesId) {
  const dbItem = yield entity.get(
    process.env.SERVERLESS_EVENT_SERIES_TABLE_NAME,
    eventSeriesId,
    true
...
module.exports.createOrUpdateEventSeries = co.wrap(
  function*(existingEventSeriesId, params) {
