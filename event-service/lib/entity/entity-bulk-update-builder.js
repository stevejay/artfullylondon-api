...
class EntityBulkUpdateBuilder {
  ...
  addFullSearchUpdate(document, indexName) {
    ...
    const version = document.entityType === globalConstants.ENTITY_TYPE_EVENT
      ? null
      : document.version;

    this.builder.index(document, indexName, 'default', document.id, version);
    return this;
  }
  ...
  addAutocompleteSearchUpdate(document, indexName) {
    if (document.status === globalConstants.STATUS_TYPE_ACTIVE) {
      ...
      this.builder.delete(indexName, 'default', document.id, document.version);
    }
