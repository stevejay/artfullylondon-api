...
const ENTITY_TYPE_VALIDATOR = {
  presence: true,
  inclusion: entityType.ALLOWED_VALUES
};

const UPDATE_PREFERENCES_VALIDATOR = {
  preferences: {
    object: {
      emailFrequency: {
        presence: true,
...
const UPDATE_WATCHES_VALIDATOR = {
  entityType: ENTITY_TYPE_VALIDATOR,
  newVersion: {
    presence: true,
    number: true,
...
const GET_WATCHES_VALIDATOR = { entityType: ENTITY_TYPE_VALIDATOR };
...
export function validateGetWatchesRequest(request) {
  ensure(request, GET_WATCHES_VALIDATOR, errorHandler);
}

export function validateUpdatePreferencesRequest(request) {
  ensure(request, UPDATE_PREFERENCES_VALIDATOR, errorHandler);
}

export function validateUpdateWatchesRequest(request) {
  ensure(request, UPDATE_WATCHES_VALIDATOR, errorHandler);
}
